...
const getSingleSemesterRegistration = catchAsync(
  async (req: Request, res: Response) => {
    ...
    sendResponse(res, {
      statusCode: httpStatus.OK,
      success: true,
      message: 'semesterRegistration get success',
      data: result,
    });
  }
);
...
const getMyCourse = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await semesterRegistrationService.getMyCourse(userId);
...
const startMyCourse = catchAsync(async (req: Request, res: Response) => {
  const id = req.params?.id;

  const result = await semesterRegistrationService.startMyCourse(id);
...
const getMySemesterRegistrationCourses = catchAsync(
  async (req: Request, res: Response) => {
    const result = await semesterRegistrationService.getSemesterRegCourse(
      req.user?.userId
    );
